...
![image-20230402112023285](./cs438_hw3.assets/image-20230402112023285.png)
...
> In the context of routing tables, "Next Hop" refers to the immediate neighbor a node should forward a packet to in order to reach a specific destination. It is the next node or router in the path to the final destination.
>
> When a packet arrives at a router or a node, it checks its routing table to determine the best path to forward the packet towards its destination. The "Next Hop" column in the routing table provides this information. By sending the packet to the next hop, the node ensures that the packet is one step closer to its final destination, following the shortest path calculated by the routing algorithm used (such as Dijkstra's algorithm or the distance vector algorithm).
>
> In summary, the next hop is the node's immediate neighbor that should receive the packet as it moves along the optimal path to the destination.
...
![image-20230401201615672](./cs438_hw3.assets/image-20230401201615672.png)
